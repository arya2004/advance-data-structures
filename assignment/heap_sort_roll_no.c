...
// Function to heapify an array at index 'parent' of size 'size' based on roll numbers
void heapify(Student* arr, int parent, int size) {
    int left = 2 * parent + 1;
    int right = 2 * parent + 2;
    int largest = parent;

    // Check if left child exists and has a larger roll number than the parent
    if (left < size && arr[left].roll_no > arr[largest].roll_no) {
        largest = left;
    }

    // Check if right child exists and has a larger roll number than the largest so far
...
int main() {
    ...
    // Build a max heap based on roll numbers
    int size = n;
    for (int i = (size / 2) - 1; i >= 0; i--) {
        heapify(students, i, size);
    }

    // Heap sort based on roll numbers
    ...
    printf("\nStudents' Information (Sorted by Roll No):\n");
    for (int i = 0; i < n; i++) {
        printf("Name: %s\tRoll No: %d\tCPI: %.2f\n", students[i].name, students[i].roll_no, students[i].CPI);
    }
